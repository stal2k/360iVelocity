	...
	function show(tab_name) {
		...
		switch(tab_name) {
			case 'video' :
				...
				jQuery('#extra-fields').load(M360IVelocity.pressThisUrl, { ajax: 'video', s: M360IVelocity.content}, function() {
					jQuery('#embed-code').prepend(M360IVelocity.content);
				});
			...
			case 'photo' :
				...
				if(photostorage == false) {
					jQuery.ajax({
						type: "GET",
						cache : false,
						url: M360IVelocity.pressThisUrl,
						data: "ajax=photo_js&u="+M360IVelocity.urlEncoded,
						dataType : "script",
